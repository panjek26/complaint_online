...
<?php include ('side_menu.php') // menu?>
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-xs-12">
                        <h3 class="page-header">Complaint >> Pilih Complaint</h3>
                    </div>
                    <!-- /.col-lg-12 -->
					 <div class="col-xs-12">
...
<?php } ?>
<?php if($_SESSION['level']<>'SPV' && $_SESSION['Staff']){ //sengaja di buat salah algoritma nya supaya tidak tampil?>
								<div class="form-group">
...
if($_POST['submit']=="Simpan"){
	...
	$nik 		=$_SESSION['nik'];
	$tgl		=date('Y-m-d H:i:s'); // pantek sni
	$folder		="attachment/";
	$id			=$_POST['id'];
	$assign		=$_POST['assign'];
	...
	echo "<script>alert('MAAF NAMA FILE SUDAH ADA SEBELUMNYA'); history.back()</script>";
		...
		if($status=="Finished"){ //jika status finish hitung di bawah ini :
			$cek_closed=$fungsi->log_complaint($id);
			$hitung=strtotime($tgl); //tanggal sekarang pantek variabel tanggal di atas
			while($cek_cl=mysql_fetch_array($cek_closed)){ 
				if($cek_cl['status']=='Working'){ // ubah ini kalo mau di ganti waktu mulai hitung nya
					$hitung -= strtotime($cek_cl['tanggal']); //mengurani nilai kiri dengan nilai kanan, waktu sekarang di kurangi waktu pengerjaan lakukang perulangan
				} else if ($cek_cl['status']=='Pending'){
					$hitung += strtotime($cek_cl['tanggal']); //menambahkan nilai kiri dengan nilai kanan untuk fungsi pause waktu SLA, waktu sekarang di tambah waktu melakukan pending, untuk mem-pause
				}
			}
			$total = floor($hitung/60); //floor pembulatan desimal
			$selisih_data= $fungsi->get_selisih($id);
			$sla_time=mysql_fetch_array($selisih_data);
			$sla_t=$sla_time['sla'];
			if($sla_t>=$total) { //jika waktu sla masih lebih besar dari waktu finished berarti sla tercapai
				$status_sla="YES";
			} else {
				$status_sla="NO"; // klo enggak tinggal di no
			}
		}

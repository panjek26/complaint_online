...
class Fungsi
{
	...
	if($id==0){ $qry_id=" status_dept='0' "; }
		...
		$query="select * from dept where $qry_id and id_dept<>1";
		$result=mysql_query($query);
		return $result;
	}
	...
	function get_staff($status,$job){
		...
		$query="SELECT *,case when level='Client' Then 'Toko' else level end as level_ganti FROM staff a
	...
	if($id==0){ $qry_id=" status_category='0' "; }
		...
		$query="select * from `category` where $qry_id order by sla asc";
		$result=mysql_query($query);
		return $result;
	}
	...
	function st_complaint($id,$job){
		...
		$query="select c.nama,b.nama_category,a.* from complaint a
				...
				inner join staff c on a.created_by=c.nik $where && a.status<>'Close' order by b.sla desc, created_date asc";
		$result=mysql_query($query);
		return $result;
	}
	...
	function log_complaint($id){ // fungsi log complaint class diagram beda field
		$query="select b.nama,case when b.level='Client' then 'Toko' else b.level end as level,a.* from log_complaint a
	...
	function get_lap_dept($start_date,$end_date){
		...
		$query="select $column a.nama_dept,b.nama,a.id_dept,c.no_ticket, sum(if(created_date between '$start_date' and '$end_date',1,0)) as ttl
				...
				where a.status_dept=1 and a.id_dept<>1 group by a.nama_dept
	...
	function upd_complaint_st($attach, $rincian, $id, $assign, $stat_lama, $status, $solusi, $status_sla, $ktgr){
		...
		if(!empty($assign) && $stat_lama=='Open'){
			...
			if(!empty($assign) && $stat_lama=='Assignment'){           // bagian ini hapus
			$qry_assign="assigment='$assign', status='Assignment', "; // bagian ini di rubah klo  mau balik ke awal jadi $qry_assign=""
		}
		}   //ini juga hapus
		if(!empty($status)){

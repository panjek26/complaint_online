...
<?php } ?>
								...
								<div class="form-group">
									<label>ATTACHMENT (Format Gambar JPEG,JPG,PNG)</label>
									<input type="file" name="attach" ></input>
								</div>
								<div class="form-group">
                                    <label>RINCIAN MASALAH</label>
                                    <textarea class="form-control" placeholder="Detail Komplain Yang Ingin Disampaikan........" rows="3" name="masalah" required ></textarea>
                                </div>
								<div class="col-md-2">
...
if($_POST['submit']=="Create Complaint"){
	...
	$extension		= end($ext);
if (in_array($extension, $allowedExts)or empty ($attach)) {
	...
	if(mysql_num_rows($cek)>=1){
		echo "<script>alert('Untuk Complain Kembali Mohon Tutup Komplain'); location.href='complaint_st.php'</script>";
	} else {
